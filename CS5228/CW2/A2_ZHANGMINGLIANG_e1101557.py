...
class MyDecisionTreeRegressor:
    ...
    def calc_thresholds(self, x):       
        ...
        for i in range(len(temp)-1):
            thresholds.add((temp[i]+temp[i+1])/2)
    ...
    def calc_best_split(self, X, y):
        ...
        for feature_idx in range(X.shape[1]):
            ...
            cur_thresholds = self.calc_thresholds(x)
            for th in cur_thresholds:
                idx_left, idx_right = self.create_split(x, th)
                cur_score = self.calc_rss_score_split(y[idx_left], y[idx_right])
                if cur_score < best_score:
                    best_score, best_threshold, best_feature_idx, best_split = cur_score, th, feature_idx, [idx_left, idx_right]
...
class MyRandomForestRegressor:
    ...
    def bootstrap_sampling(self, X, y):
        ...
        random_idx = np.random.choice(N, N, replace=True)
        X_bootstrap, y_bootstrap = X[random_idx, :], y[random_idx]
    ...
    def feature_sampling(self, X):
        ...
        indices_sampled = np.random.choice(d, int(d * self.max_features), replace=False)
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            ...
            regressor = MyDecisionTreeRegressor().fit(X_feature_sampled, y_bootstrap)
    ...
    def predict(self, X):
        ...
        for estimator in self.estimators:
            sampled_input = X[:, estimator[1]]
            predictions.append(estimator[0].predict(sampled_input))
